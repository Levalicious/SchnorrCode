...
public class HashUtil {
    ...
    protected static byte[] blake2(byte[] input) {
        blake.reset();
        return blake.digest(input);
    }

    protected static byte[] sha3(byte[] input) {
        keccak.reset();
        return keccak.digest(input);
    }

    protected static byte[] blake2omit12(byte[] input) {
        byte[] hash = blake2(input);
        return Arrays.copyOfRange(hash, 12, hash.length);
    }

    protected static byte[] blake2ECC(byte[] input) {
        byte[] hash = blake2(input);
